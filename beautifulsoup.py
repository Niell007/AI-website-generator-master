...
def generate_images(html_content, local_directory):
    ...
    for image_element in image_elements:
        ...
        image_path = generate_image(style, alt, img_src, local_directory)
    ...
    for style_element in style_elements:
        ...
        if css_content:
            css_urls = re.findall(r'url\(["\']?(?P<url>[^\)]+?)["\']?\)', css_content)
            for css_url in css_urls:
                image_path = generate_image(
                    None,
                    "Generate a beautiful realistic photo for a background picture of a banner:" + css_url,
                    css_url,
                    local_directory
                )
                image_name = image_path.split("/")[-1]
